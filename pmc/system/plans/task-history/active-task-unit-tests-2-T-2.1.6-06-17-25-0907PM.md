# T-2.1.6: Breakpoint System Extraction - Enhanced Testing Protocol

## Mission Statement
Execute complete testing cycle from environment setup through visual validation with LLM Vision analysis to ensure T-2.1.6 components (T-2.1.6:ELE-1, T-2.1.6:ELE-2, T-2.1.6:ELE-3) are properly implemented, styled, and functioning with component functionality.
...
```bash
# PURPOSE: Create the complete directory structure required for T-2.1.6 testing artifacts
# WHEN: Run this before any testing phases to ensure all output directories exist
# PREREQUISITES: You are in aplio-modern-1/ directory
# EXPECTED OUTCOME: All required test directories exist for T-2.1.6 components
# FAILURE HANDLING: If mkdir fails, check permissions and available disk space

mkdir -p test/unit-tests/task-2-1.6/T-2.1.6
mkdir -p test/screenshots/T-2.1.6
mkdir -p test/scaffolds/T-2.1.6
mkdir -p test/references/T-2.1.6
mkdir -p test/diffs
...
- [ ] All T-2.1.6 test directories created
- [ ] Test server running on port 3333
- [ ] Dashboard running on port 3334
- [ ] All testing dependencies installed

### Deliverables
- Complete test directory structure for T-2.1.6
...
```bash
# PURPOSE: Discover all testable elements created by T-2.1.6 and classify their testing approach using AI-powered analysis
...
# - Task: T-2.1.6 - Breakpoint System Extraction
...
# - Element Preview: Shadow system extraction: Create TypeScript definitions for shadows with elevation levels

# Targeted Analysis Process:
# 1. Focus on Components/Elements Section: Review the 3 elements starting with: Shadow system extraction: Create TypeScript definitions for shadows with elevation levels
# 2. Examine Implementation at: `C:\Users\james\Master\BrightHub\Build\APSD-runs\aplio-27-a1-c\aplio-modern-1\styles\design-tokens\breakpoints.ts` with pattern P006-DESIGN-TOKENS
...
echo "Task: T-2.1.6 - Breakpoint System Extraction"
echo "Pattern: P006-DESIGN-TOKENS"
echo "Elements Count: 3"
echo "Implementation Location: `C:\Users\james\Master\BrightHub\Build\APSD-runs\aplio-27-a1-c\aplio-modern-1\styles\design-tokens\breakpoints.ts`"
echo ""
echo "Analyzing Shadow system extraction: Create TypeScript definitions for shadows with elevation levels and related testable elements..."
...
#### Step 1.2: Discover and Validate T-2.1.6 Components
```bash
# PURPOSE: Validate that all T-2.1.6 components can be imported and compiled
...
# PREREQUISITES: Component importer system available, all T-2.1.6 components implemented
# EXPECTED OUTCOME: All 3 T-2.1.6 components successfully imported and validated
...
const T216Components = ['T-2.1.6:ELE-1', 'T-2.1.6:ELE-2', 'T-2.1.6:ELE-3'];

async function validateAllComponents() {
  for (const name of T216Components) {
    try {
  ...
  console.log('All T-2.1.6 components validated');
}
...
#### Step 1.3: Generate Enhanced Scaffolds for All T-2.1.6 Components
```bash
# PURPOSE: Generate React SSR scaffolds with real rendering, Tailwind CSS, and visual boundaries for all T-2.1.6 components
...
# EXPECTED OUTCOME: 3 enhanced scaffold HTML files created in test/scaffolds/T-2.1.6/ with real React content
...
const components = [
  { name: 'T-2.1.6:ELE-1', type: 'server', props: {"title":"Test T-2.1.6:ELE-1 for T-2.1.6","children":"Test content for T-2.1.6:ELE-1 in T-2.1.6"} },
  { name: 'T-2.1.6:ELE-2', type: 'server', props: {"title":"Test T-2.1.6:ELE-2 for T-2.1.6","children":"Test content for T-2.1.6:ELE-2 in T-2.1.6"} },
  { name: 'T-2.1.6:ELE-3', type: 'server', props: {"title":"Test T-2.1.6:ELE-3 for T-2.1.6","children":"Test content for T-2.1.6:ELE-3 in T-2.1.6"} }
];

async function generateAllScaffolds() {
  for (const component of components) {
    try {
      const path = await createEnhancedScaffold({ task: 'T-2.1.6', component: component.name, props: component.props });
  ...
  console.log('All T-2.1.6 scaffolds generated');
}
...
# PREREQUISITES: Enhanced scaffolds generated in test/scaffolds/T-2.1.6/
...
find test/scaffolds/T-2.1.6 -name "*-enhanced.html" -exec grep -L "Mock\|placeholder\|test content" {} \; | while read file; do echo "✓ $file contains real content"; done

# Verify Tailwind CSS classes are present
find test/scaffolds/T-2.1.6 -name "*-enhanced.html" -exec grep -l "bg-white\|rounded-lg\|shadow-md\|bg-blue\|bg-green" {} \; | while read file; do echo "✓ $file has Tailwind CSS"; done

# Check for proper component boundaries
find test/scaffolds/T-2.1.6 -name "*-enhanced.html" -exec grep -l "Server Component\|Client Component\|component-boundary" {} \; | while read file; do echo "✓ $file has visual boundaries"; done
```

### Validation
- [ ] All 3 T-2.1.6 components successfully discovered and classified
...
- 3 enhanced scaffold HTML files in test/scaffolds/T-2.1.6/
...
# DOCUMENTATION: You MUST read all of C:\Users\james\Master\BrightHub\Build\APSD-runs\aplio-27-a1-c\pmc\system\plans\task-approach\current-test-discovery.md because all testable elements have been documented there.
# PREREQUISITES: Jest installed, test files exist in test/unit-tests/task-2-1.6/T-2.1.6/, components discovered in Phase 1
...
npm test -- --testPathPattern=task-2-1-6/T-2.1.6 --coverage
```
...
# PREREQUISITES: All T-2.1.6 component files discovered in Phase 1, components exist in `C:\Users\james\Master\BrightHub\Build\APSD-runs\aplio-27-a1-c\aplio-modern-1\styles\design-tokens\breakpoints.ts`
...
#### Step 2.3: Create Unit Test Files for T-2.1.6
```bash
# PURPOSE: Generate comprehensive unit test files for server and client component validation
# WHEN: Run this if unit test files don't exist for discovered T-2.1.6 components
# DOCUMENTATION: You MUST read all of C:\Users\james\Master\BrightHub\Build\APSD-runs\aplio-27-a1-c\pmc\system\plans\task-approach\current-test-discovery.md because all testable elements have been documented there.
# PREREQUISITES: test/unit-tests/task-2-1.6/T-2.1.6/ directory exists, components discovered in Phase 1
...
### Validation
- [ ] All Jest unit tests pass for discovered T-2.1.6 components
...
### Deliverables
- Jest test results with coverage for T-2.1.6
...
- Enhanced scaffolds generated for all T-2.1.6 components
- Test server running on port 3333
- Scaffolds contain real React content with styling

### Actions

#### Step 3.1: Execute Enhanced Visual Testing for T-2.1.6
```bash
# PURPOSE: Capture pixel-perfect screenshots of all T-2.1.6 components using Playwright
...
# EXPECTED OUTCOME: High-quality PNG screenshots captured for all 3 T-2.1.6 components
# FAILURE HANDLING: If visual testing fails, restart test server and check scaffold accessibility

npm run test:visual:enhanced T-2.1.6
```

#### Step 3.2: Validate Screenshot Generation
```bash
# PURPOSE: Verify all expected T-2.1.6 component screenshots were successfully captured
# WHEN: Run this after visual testing to confirm all artifacts are ready for LLM Vision analysis
# PREREQUISITES: Visual testing completed, test/screenshots/T-2.1.6/ directory exists
# EXPECTED OUTCOME: 3 PNG screenshot files confirmed for T-2.1.6 components
# FAILURE HANDLING: If screenshots missing, re-run visual testing for missing components

node -e "
const fs = require('fs');
const screenshotDir = 'test/screenshots/T-2.1.6';
const expectedComponents = ['T-2.1.6:ELE-1', 'T-2.1.6:ELE-2', 'T-2.1.6:ELE-3'];
...
if (!allValid) {
  throw new Error('Some T-2.1.6 component screenshots are missing');
}
console.log('All T-2.1.6 component screenshots validated');
"
```
...
### Validation
- [ ] All 3 T-2.1.6 component screenshots captured
...
### Deliverables
- 3 PNG screenshot files in test/screenshots/T-2.1.6/
...
#### Step 4.2: Execute Enhanced LLM Vision Analysis for All T-2.1.6 Components
```bash
# PURPOSE: Run Enhanced LLM Vision analysis on each T-2.1.6 component to validate content and classification
...
COMPONENTS=("T-2.1.6:ELE-1" "T-2.1.6:ELE-2" "T-2.1.6:ELE-3")
...
for component in "${COMPONENTS[@]}"; do
  ...
  if [ "$component" != "T-2.1.6:ELE-3" ]; then
...
```bash
# PURPOSE: Verify all T-2.1.6 components have comprehensive analysis reports with acceptable confidence scores
...
# EXPECTED OUTCOME: 3 detailed analysis reports confirmed in test/screenshots/T-2.1.6/
# FAILURE HANDLING: If reports missing or confidence low, re-run analysis with improved prompts

COMPONENTS=("T-2.1.6:ELE-1" "T-2.1.6:ELE-2" "T-2.1.6:ELE-3")

for component in "${COMPONENTS[@]}"; do
  report_path="test/screenshots/T-2.1.6/${component}-enhanced-analysis.md"
...
- [ ] All 3 T-2.1.6 components analyzed successfully
...
### Deliverables
- 3 detailed LLM Vision analysis reports in test/screenshots/T-2.1.6/
...
#### Step 5.1: Compile T-2.1.6 Testing Results
```bash
# PURPOSE: Generate comprehensive summary of all T-2.1.6 testing phase results
...
# EXPECTED OUTCOME: Complete testing summary with pass/fail status for all T-2.1.6 components
# FAILURE HANDLING: If compilation fails, verify all prerequisite artifacts exist

node -e "
const fs = require('fs');
const components = ["T-2.1.6:ELE-1","T-2.1.6:ELE-2","T-2.1.6:ELE-3"];

console.log('=== T-2.1.6 TESTING SUMMARY ===');
...
components.forEach(comp => {
  const scaffoldPath = `test/scaffolds/T-2.1.6/${comp}-enhanced.html`;
...
console.log('\nVISUAL TESTING:');
components.forEach(comp => {
  const screenshotPath = `test/screenshots/T-2.1.6/${comp}-enhanced.png`;
...
console.log('\nLLM VISION ANALYSIS:');
components.forEach(comp => {
  const reportPath = `test/screenshots/T-2.1.6/${comp}-enhanced-analysis.md`;
...
console.log('\n=== FINAL RESULT ===');
if (allPassed) {
  console.log('✓ ALL T-2.1.6 TESTING PHASES PASSED');
  console.log('Components ready for production validation');
} else {
  console.log('✗ SOME T-2.1.6 TESTING PHASES FAILED');
...
```bash
# PURPOSE: Create final testing report for human validation with all T-2.1.6 results and artifacts
...
cat > test/reports/T-2.1.6-testing-report.md << 'EOF'
# T-2.1.6 undefined - Testing Report

## Executive Summary
Complete testing validation for T-2.1.6 components with Enhanced LLM Vision analysis.

## Components Tested
- **T-2.1.6:ELE-1** (Server Component) - Breakpoint value extraction: Create TypeScript definitions for screen size breakpoints with blue boundaries
- **T-2.1.6:ELE-2** (Server Component) - Responsive utility helpers: Implement Next.js 14 compatible responsive utility functions with blue boundaries
- **T-2.1.6:ELE-3** (Server Component) - Breakpoint token type definitions: Create TypeScript types and interfaces for breakpoint tokens with blue boundaries
...
## Artifacts Generated
- Unit test files: `test/unit-tests/task-2-1.6/T-2.1.6/`
- Enhanced scaffolds: `test/scaffolds/T-2.1.6/`
- Screenshots: `test/screenshots/T-2.1.6/`
- LLM Vision reports: `test/screenshots/T-2.1.6/*-enhanced-analysis.md`
...
1. Visual quality meets T-2.1.6 requirements
...
echo "✓ T-2.1.6 testing report generated: test/reports/T-2.1.6-testing-report.md"
```

### Validation
- [ ] All T-2.1.6 testing phases completed successfully
...
- T-2.1.6 ready for human validation

## Success Criteria & Quality Gates

### Component Implementation Requirements
- **T-2.1.6:ELE-1 (Server)**: Proper server component with no client directive, Breakpoint value extraction: Create TypeScript definitions for screen size breakpoints, blue boundary
- **T-2.1.6:ELE-2 (Server)**: Proper server component with no client directive, Responsive utility helpers: Implement Next.js 14 compatible responsive utility functions, blue boundary
- **T-2.1.6:ELE-3 (Server)**: Proper server component with no client directive, Breakpoint token type definitions: Create TypeScript types and interfaces for breakpoint tokens, blue boundary
...
### Review Locations
- **Enhanced Scaffolds**: `test/scaffolds/T-2.1.6/` - Real React rendering with boundaries
- **Screenshots**: `test/screenshots/T-2.1.6/` - Visual component validation
- **LLM Vision Reports**: `test/screenshots/T-2.1.6/*-enhanced-analysis.md` - AI analysis
- **Testing Report**: `test/reports/T-2.1.6-testing-report.md` - Complete summary
...
4. Confirm all components meet T-2.1.6 acceptance criteria
5. Validate server/client classification through visual boundaries

### Completion Checklist
- [ ] All testing phases executed successfully
- [ ] 3 T-2.1.6 components validated through Enhanced LLM Vision analysis
...
No legacy code references available for Shadow system extraction: Create TypeScript definitions for shadows with elevation levels

### [T-2.1.5:ELE-2] Border system extraction: Document and implement border widths, radii, and styles
No legacy code references available for Border system extraction: Document and implement border widths, radii, and styles

### [T-2.1.5:ELE-3] Effects token type definitions: Create TypeScript types and interfaces for shadow and border tokens
No legacy code references available for Effects token type definitions: Create TypeScript types and interfaces for shadow and border tokens
